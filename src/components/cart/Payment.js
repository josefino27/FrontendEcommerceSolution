...
const Wrapper = (props) => (
  <Elements mercadoPago={loadStripe(localStorage.getItem("stripeapi"))}>
    <Payment {...props} />
  </Elements>
);
...
const Payment = () => {
  ...
  const submitHandler = async (e) => {
    ...
    try {
      if (!mercadoPago || !elements) {
        return;
      }

      const result = await mercadoPago.confirmCardPayment(clientSecret, {
        payment_method: {
      ...
      return (document.querySelector("#pay_btn").disabled = false);
